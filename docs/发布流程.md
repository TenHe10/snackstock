...
```bash
uv run pyinstaller --noconfirm --clean --windowed --name SnackStock main.py
```

未使用 uv 时：

```bash
pyinstaller --noconfirm --clean --windowed --name SnackStock main.py
```
...
1. 固化 `PyInstaller` 的 `.spec` 文件，避免命令漂移
2. 增强 `build_windows.bat`（版本号参数、产物自动归档、验收检查）
